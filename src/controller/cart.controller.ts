...
export async function addItem(req: AuthRequest, res: Response, next: NextFunction) {
    try {
        ...
        if (!productId) throw new AppError(400, "PRODUCT_MISSING", "productID is required")
        if (quantity < 1) throw new AppError(400, "INVALID_QUANTITY", "quantity must be a positive integer")
